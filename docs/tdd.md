...
```bash
npx skills add bradcstevens/git-loopy-skills --skill=tdd
```
...
[Source](https://github.com/bradcstevens/git-loopy-skills/tree/main/skills/tdd)
...
Reach for it when there's a concrete behaviour to build and you want tests that survive a refactor. If the behaviour isn't pinned down yet, settle the spec first — for that, use [to-spec](./to-spec.md). When the work is really about the shape of the interface rather than the tests, use [codebase-design](./codebase-design.md); `tdd` calls into it for the deep-module vocabulary during planning.
...
[implement](./implement.md) is the chain's build step, and it drives `tdd` internally to build each ticket test-first before handing off to [code-review](./code-review.md) — so `tdd` is the engine inside that step rather than a step of its own. You can also reach for it directly, whenever there's a concrete behaviour to build without a full spec. Its other neighbour is [codebase-design](./codebase-design.md), which it leans on to find deep-module seams worth testing at. When you're unsure which skill or flow fits, [next](./next.md) routes you.
