...
```bash
npx skills add bradcstevens/git-loopy-skills --skill=implement
```
...
[Source](https://github.com/bradcstevens/git-loopy-skills/tree/main/skills/implement)
...
Reach for it once the work is written down as a spec or split into tickets and you're ready to turn that into code. If the spec doesn't exist yet, write it first — for that, use [to-spec](./to-spec.md), or [to-tickets](./to-tickets.md) to break a spec into tickets. If you just want to build something test-first without a full spec, drop to [tdd](./tdd.md) directly.

## Pre-agreed seams

The idea `implement` runs on is the **seam** — the stable interface a feature is tested at, chosen before any code is written. It doesn't invent seams mid-build; it uses the ones already picked (during [to-spec](./to-spec.md)) and writes tests against them via [tdd](./tdd.md). Working at pre-agreed seams is what keeps the implementation honest: the tests target something durable, so the code underneath can move without the tests moving.
