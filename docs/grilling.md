...
```bash
npx skills add bradcstevens/git-loopy-skills --skill=grilling
```
...
`grilling` is the **single source of truth** for the interview technique, split out as a model-invoked **primitive** so every skill that needs an interview can reach it instead of reinventing one. [grill-me](https://github.com/bradcstevens/git-loopy-skills/tree/main/skills/grill-me) and [grill-with-docs](./grill-with-docs.md) are its two user-invoked front doors, but [improve-codebase-architecture](./improve-codebase-architecture.md) and [triage](./triage.md) also lean on it to pressure-test their own decisions.
