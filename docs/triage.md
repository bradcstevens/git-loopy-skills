...
```bash
npx skills add bradcstevens/git-loopy-skills --skill=triage
```
...
[Source](https://github.com/bradcstevens/git-loopy-skills/tree/main/skills/triage)
...
Reach for it when your issue tracker has raw, unevaluated reports and you want them sorted, verified, and turned into work an agent or human can pick up. To turn a settled conversation into a fresh spec instead, use [to-spec](./to-spec.md); to split an existing spec into tickets, use [to-tickets](./to-tickets.md). `triage` is the reverse direction — it processes what's *already* landed in the tracker.

## Prerequisites

`triage` reads and writes your issue tracker, so [setup-git-loopy-skills](./setup-git-loopy-skills.md) must have configured the tracker and the label mapping first. The role names above are **canonical** — the actual label strings in your tracker may differ, and that mapping is what setup provides. The config also decides whether external PRs count as a request surface, and who counts as external.
...
`triage` is the **periodic maintenance** pass over your issue tracker — run it whenever reports pile up, to keep the queue sorted and the `ready-for-agent` column trustworthy. It sits at the front of the tracker, upstream of the build chain: the briefs it writes are what [tdd](./tdd.md) later picks up to implement. When a request needs sharpening it leans on [grilling](./grilling.md) and [domain-modeling](./domain-modeling.md) to grill it into shape a round of questions at a time. Its close neighbour is [to-spec](./to-spec.md), which populates the tracker from a fresh conversation where `triage` processes what's already there. When you're unsure which skill or flow fits, [next](./next.md) routes you.
