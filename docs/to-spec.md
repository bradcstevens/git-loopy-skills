...
`to-spec` publishes into your issue tracker, so [setup-agent-skills](./setup-agent-skills.md) must have configured the tracker and triage labels for this repo first. It applies the `ready-for-agent` label itself — no separate triage pass needed.
