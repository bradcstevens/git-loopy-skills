...
```bash
npx skills add bradcstevens/git-loopy-skills --skill=research
```
...
[Source](https://github.com/bradcstevens/git-loopy-skills/tree/main/skills/research)
...
Reach for it when the next step is *finding something out* — how an API behaves, what a spec actually says, whether a claim holds — and you'd rather not stall your own thread doing the reading. For sharpening a plan by interview instead of by reading, use [grilling](./grilling.md); for exploring what to build with throwaway code, use [prototype](./prototype.md).
