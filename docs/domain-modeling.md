...
```bash
npx skills add bradcstevens/git-loopy-skills --skill=domain-modeling
```
...
[Source](https://github.com/bradcstevens/git-loopy-skills/tree/main/skills/domain-modeling)
...
Reach for it when the *words* are the problem: two people mean different things by "cancellation", "account" is doing three jobs, or a design conversation keeps snagging on a concept that has never been named precisely. If instead the module's *shape* is the problem — where the seam goes, how deep the interface is — use [codebase-design](./codebase-design.md). If you want the plan itself interrogated before you build, use [grilling](./grilling.md).
...
`domain-modeling` is the **single source of truth** for building the project's ubiquitous language, split out as its own model-invoked skill so any other skill can reach it. [grill-with-docs](./grill-with-docs.md) leans on it to record terms and decisions as a grilling session runs, [triage](./triage.md) uses it to keep tickets in the project's own words, and [improve-codebase-architecture](./improve-codebase-architecture.md) reaches for it while it works.
