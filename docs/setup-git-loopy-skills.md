...
> otherwise a fresh clone can appear to have a broken trigger with no error.

## It's working if

- `issue-tracker.md` and `domain.md` land under `docs/agents/` (plus `triage-labels.md` when `triage` is installed), an `## Agent skills` section appears in your `AGENTS.md`, and `.github/hooks/git-loopy-chain.json` contains the installed chain script path.
