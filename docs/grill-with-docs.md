...
```bash
npx skills add bradcstevens/git-loopy-skills --skill=grill-with-docs
```
...
[Source](https://github.com/bradcstevens/git-loopy-skills/tree/main/skills/grill-with-docs)
...
Reach for it at the very start of a change, when the plan is still fuzzy and the domain language isn't settled, and you want to stress-test both before any code exists. If you only want the interview and don't need the artifacts, use [grilling](./grilling.md); if the plan is already clear and you just need to pin down or record terminology, use [domain-modeling](./domain-modeling.md). And if the change is too big to hold in one session and its route is still foggy — a greenfield project, a huge feature build — start upstream with [wayfinder](./wayfinder.md): it charts the effort as a map of decisions, then hands back to this main flow once the way is clear.
